...
namespace NoteEditor.Notes
{
    public class NoteObject : IDisposable
    {
        ...
        Color singleNoteColor = new Color(255 / 255f, 255 / 255f, 255 / 255f); //シングルノーツカラー
        Color longNoteColor = new Color(0 / 255f, 255 / 255f, 255 / 255f); //ロングノーツカラー
        Color attackNoteColor = new Color(255 / 255f, 0 / 255f, 0 / 255f); //アタックノーツカラー
        Color coinNoteColor = new Color(255 / 255f, 255 / 255f, 0 / 255f); //コインノーツカラー
        Color jumpNoteColor = new Color(255 / 255f, 69 / 255f, 0 / 255f); //ジャンプノーツカラー
        ...
        public void Init()
        {
                ...
                .Subscribe(type =>
                {
                    switch (type)
                    {
                        case NoteTypes.Single:
                            noteColor_.Value = singleNoteColor; //シングルノーツ
                            break;
                        case NoteTypes.Long:
                            noteColor_.Value = longNoteColor; //ロングノーツ
                            break;
                        case NoteTypes.Attack:
                            noteColor_.Value = attackNoteColor; //アタックノーツ
                            break;
                        case NoteTypes.Coin:
                            noteColor_.Value = coinNoteColor; //コインノーツ
                            break;
                        case NoteTypes.Jump:
                            noteColor_.Value = jumpNoteColor; //ジャンプノーツ
                            break;
                        default:
                            noteColor_.Value = invalidStateColor;
                            break;
                    }
            ...
            //アタックノーツ
            disposable.Add(mouseDownObservable.Where(editType => editType == NoteTypes.Attack)
                .Where(editType => editType == noteType.Value)
                .Subscribe(_ => editPresenter.RequestForRemoveNote.OnNext(note)));

            //コインノーツ
            disposable.Add(mouseDownObservable.Where(editType => editType == NoteTypes.Coin)
                .Where(editType => editType == noteType.Value)
                .Subscribe(_ => editPresenter.RequestForRemoveNote.OnNext(note)));

            //ジャンプノーツ
            disposable.Add(mouseDownObservable.Where(editType => editType == NoteTypes.Jump)
                .Where(editType => editType == noteType.Value)
                .Subscribe(_ => editPresenter.RequestForRemoveNote.OnNext(note)));
        ...
        public void SetState(Note note)
        {
            ...
            //アタックノーツ
            if (note.type == NoteTypes.Attack)
            {
                RemoveLink();
            }

            //コインノーツ
            if (note.type == NoteTypes.Coin)
            {
                RemoveLink();
            }

            //ジャンプノーツ
            if (note.type == NoteTypes.Jump)
            {
                RemoveLink();
            }
        }
