...
namespace NoteEditor.Presenter
{
    public class EditNotesPresenter : SingletonMonoBehaviour<EditNotesPresenter>
    {
        ...
        void Init()
        {
            ...
            //アタックノーツ
            closestNoteAreaOnMouseDownObservable
                .Where(_ => EditState.NoteType.Value == NoteTypes.Attack)
                .Subscribe(_ =>
                {
                    if (EditData.Notes.ContainsKey(NoteCanvas.ClosestNotePosition.Value))
                    {
            ...
            //コインノーツ
            closestNoteAreaOnMouseDownObservable
                .Where(_ => EditState.NoteType.Value == NoteTypes.Coin)
                .Subscribe(_ =>
                {
                    if (EditData.Notes.ContainsKey(NoteCanvas.ClosestNotePosition.Value))
                    {
            ...
            //ジャンプノーツ
            closestNoteAreaOnMouseDownObservable
                .Where(_ => EditState.NoteType.Value == NoteTypes.Jump)
                .Subscribe(_ =>
                {
                    if (EditData.Notes.ContainsKey(NoteCanvas.ClosestNotePosition.Value))
                    {
            ...
            RequestForEditNote.Subscribe(note =>
            {
                ...
                //アタックノーツ
                else if (note.type == NoteTypes.Attack)
                {
                    (EditData.Notes.ContainsKey(note.position)
                             ? RequestForRemoveNote
                             : RequestForAddNote)
                ...
                //コインノーツ
                else if (note.type == NoteTypes.Coin)
                {
                    (EditData.Notes.ContainsKey(note.position)
                             ? RequestForRemoveNote
                             : RequestForAddNote)
                ...
                //ジャンプノーツ
                else if (note.type == NoteTypes.Jump)
                {
                    (EditData.Notes.ContainsKey(note.position)
                             ? RequestForRemoveNote
                             : RequestForAddNote)
